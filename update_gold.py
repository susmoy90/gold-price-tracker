import requests
import os
import datetime

def get_bdt_rate():
    try:
        # লাইভ কারেন্সি এপিআই থেকে ডলার রেট সংগ্রহ
        response = requests.get("https://api.exchangerate-api.com/v4/latest/USD")
        data = response.json()
        return data.get('rates', {}).get('BDT', 122.5)
    except:
        return 122.5

def get_gold_price():
    api_key = os.getenv("GOLD_API_KEY")
    url = "https://www.goldapi.io/api/XAU/USD"
    headers = {"x-access-token": api_key, "Content-Type": "application/json"}
    
    try:
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        gold_data = response.json()
        
        p24k_usd = gold_data.get('price_gram_24k', 0)
        usd_to_bdt = get_bdt_rate()
        v_gm = 11.664 # ১ ভরি = ১১.৬৬৪ গ্রাম

        def f_bdt(val):
            return "{:,.0f}".format(val)

        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # সাইটের টাইটেল আপডেট করা হয়েছে
        output = f"# 💰 বাংলাদেশ গোল্ড হোলসেল মার্কেট আপডেট\n"
        output += f"**শেষ আপডেট:** {current_time} | **ডলার রেট:** 1$ = {usd_to_bdt} BDT\n\n"
        output += f"> **তথ্যসূত্র:** আন্তর্জাতিক স্পট মার্কেট এবং স্থানীয় পাইকারি দরের সমন্বয়।\n\n"

        # টেবিল ১: পাইকারি বাজার (Wholesale)
        output += "### ⚖️ পাইকারি বাজারের দাম (Wholesale Price)\n\n"
        output += "| ক্যারেট | বিশুদ্ধতা | প্রতি গ্রাম (BDT) | প্রতি ভরি (১১.৬৬৪ গ্রাম) |\n"
        output += "| :--- | :---: | :---: | :---: |\n" # এখানে অ্যালাইনমেন্ট ঠিক করা হয়েছে
        
        # টেবিল ২: খুচরা দাম
        retail_table = "\n### 🛍️ লোকাল কাস্টমার খুচরা দাম (২০% প্রিমিয়ামসহ)\n\n"
        retail_table += "| ক্যারেট | প্রতি গ্রাম (BDT) | প্রতি ভরি (BDT) |\n"
        retail_table += "| :--- | :---: | :---: |\n"

        carats = [
            ("২৪ ক্যারেট", 1.0), 
            ("২২ ক্যারেট", 22/24), 
            ("২১ ক্যারেট", 21/24), 
            ("১৮ ক্যারেট", 18/24)
        ]

        for name, ratio in carats:
            b_gm = p24k_usd * ratio * usd_to_bdt
            wholesale_vhori = b_gm * v_gm
            
            retail_gram = b_gm * 1.20
            retail_vhori = wholesale_vhori * 1.20
            
            output += f"| **{name}** | {round(ratio*100, 1)}% | {f_bdt(b_gm)} ৳ | {f_bdt(wholesale_vhori)} ৳ |\n"
            retail_table += f"| **{name}** | {f_bdt(retail_gram)} ৳ | **{f_bdt(retail_vhori)} ৳** |\n"
        
        disclaimer = "\n---\n**সতর্কবার্তা:** এই মূল্যসমূহ আন্তর্জাতিক তথ্যের ওপর ভিত্তি করে ক্যালকুলেট করা। স্থানীয় জুয়েলারি দোকান বা বাজারভেদে দামের সামান্য তারতম্য হতে পারে। বড় লেনদেনের আগে বাজার যাচাই করে নিন।"
        
        return output + retail_table + disclaimer + "\n"
    except Exception as e:
        return f"Error: {e}\n"

def write_to_file(content):
    with open("index.md", "w", encoding="utf-8") as f:
        f.write(content)

if __name__ == "__main__":
    write_to_file(get_gold_price())
